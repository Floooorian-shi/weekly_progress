...
![Zyzzyva](figures/Zyzzyva_protocol.png)
...
# 六. Tendermint
## Model
  - 每个process有一定的投票权（最低为0），投票权可以不想等
  - 进程不属于同一个administrative domain，无法在所有进程之间强制建立直接网络连接。each process is connected to a subset of processes called peers, such that there is an indirect communication channel between all correct processes
  ...
  - 一旦消息日志中包含使相应条件求值为true的消息,该触发规则即被触发.
  - $n > 3f$. 考虑$n = 3f + 1$

## State in a node
  - height($h$): 当前共识实例、区块高度
  - round($r$)：当前高度中的轮次，初始为0
  - step: 当前轮的propose, prevote, precommit阶段
  - lockedValue, lockedRound：最近一次为非空值发送precommit前，锁住的值和轮次
  - validValue, validRound：最近一次看到“提案和$2f+1$个同值prevote”的可决定值及其轮次
  - 消息日志：已发送和已收到的签名消息

## process
以高度$h$，轮次$r$，正确proposer提议有效区块$B$为例。
  - Start Round
    - 一个proposer suggests a value with a PROPOSAL消息给所有进程
      - 仅当$validValue = nil$时，proposer提议一个新值，否则$validValue$被提议
      - PROPOSAL消息包含$validRound$
    - 非proposer调度timeoutPropose函数
      - 超时前，只接受$height = h_p$、$round = round_p$、$step_p = propose$的提议，广播PREVOTE消息（值为nil）并将$step_p$设置为prevote
  - 接收PROPOSAL消息且无历史证明（值为nil）
    - $valid(v) ∧ (lockedRound_p = −1 ∨ lockedValue_p = v)$时，（$lockedRoundp = −1$指$p$尚未锁定任何值），接受针对v的提议，广播PREVOTE消息（值为id(v)）
    - 否则广播PREVOTE消息（值为nil）
    - 将$step_p$设置为$prevote$
  - 接收PROPOSAL消息且有历史证明（值为$vr$，$0 \leq vr < v$）
    - 进程在自己的消息日志中看到第$vr$轮的$<PREVOTE, h_p,vr,id(v)>$数量是否大于$2f+1$，再判断$valid(v)$且满足$lockedRound_p≤vr$或$lockedValue_p=v$。
      - 满足，广播PREVOTE（值为id(v)）
      - 不满足，广播PREVOTE（值为nil）
    - 将$step_p$设置为$prevote$
  - PREVOTE
    - 第一次收到任意PREVOTE消息，并且$step_p = prevote$
      - 调度$OnTimeoutPrevote(h_p, round_p)$，使其在$timeoutPrevote(round_p)%后运行。
    - 正确进程收到针对$v$的PROPOSE消息和$2f+1$条针对id(v)的PREVOTE消息时
      - 如果$step_p = prevote$, 
        - $lockedValue_p = v$, $lockedRound_p = round_p$
        - 广播PRECOMMIT消息（值为id(v)）
      - $validValue_p = v$, $validRound_p = roung_p$
    - 收到$2f+1$条值为nil的PREVOTE消息且$step_p = prevote$时
      - 广播值为nil的PRECOMMIT消息
      - $step_p = precommit$
  - PRECOMMIT
    - 第一次收到任意PRECOMMIT消息
      - 调度$OnTimeoutPrecommit(h_p, round_p)$，使其在$timeoutPrecommit(round_p)%后运行。
    - 收到proposer的PROPOSAL消息后、收到$2f+1$条值为id(v)的PRECOMMIT消息前，$decisionp[hp] = nil$时持续做：
      - 满足$valid(v)：
        - $decisionp[hp] = v$
        - $h_p = h_p + 1$
        - 重置四个变量
...
![Tendermint_Algorithm1_1](figures/Tendermint_Algorithm1_1.png)
![Tendermint_Algorithm1_1](figures/Tendermint_Algorithm1_2.png)